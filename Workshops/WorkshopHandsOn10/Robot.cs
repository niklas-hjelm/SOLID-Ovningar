...
namespace WorkshopHandsOn10
{
    public class Robot : IRobot
    {
        ...
        public void Move(IList<Position> route)
        {
            ...
            {
                this._servicebus.Publish<SendSMSCommand>(new SendSMSCommand(new SMS(string.Format("Robot{0} is moving!", this.Name)), this.ID));
                foreach (Position p in route)
                {
                    this.MoveTo(p);
                }
                this._servicebus.Publish<SendSMSCommand>(new SendSMSCommand(new SMS(string.Format("Robot{0} has reached his goal!", this.Name)), this.ID));
            }
        }
